...
function updateTask($taskId, $data) {
    ...
    $url = "http://micro_task:3007/api/task/updateById/" . $taskId;
...
function updateUser($userId, $data) {
    ...
    $url = "http://micro_user:3009/api/user/actualizar?id=" . $userId;
...
function deleteTask($taskTitle, $taskId) {
    ...
    $url = "http://micro_task:3007/api/task/delete/" . urlencode($taskTitle) . "/" . $taskId;
...
function deleteUser($userId) {
    return callAPI("DELETE", "http://micro_user:3009/api/user/" . $userId);
}
...
function deleteProject($projectId) {
    ...
    return callAPI("DELETE", "http://micro_projects:3008/api/project/delete/" . $projectId);
}
...
function createProject($nombre, $descripcion, $creador, $miembros = []) {
    ...
    $resultado = callAPI("POST", "http://micro_projects:3008/api/project/", $data);
...
function userLogin($email, $password) {
    ...
    return callAPI("POST", "http://micro_user:3009/api/user/login", $data);
}
...
function userRegister($nombre, $email, $password) {
    ...
    return callAPI("POST", "http://micro_user:3009/api/user/registro", $data);
}

function getUserById($userId) {
    return callAPI("GET", "http://micro_user:3009/api/user/buscar", ["id" => $userId]);
}

function getUserByEmail($email) {
    return callAPI("GET", "http://micro_user:3009/api/user/buscar", ["email" => $email]);
}

function getAllUsers() {
    return callAPI("GET", "http://micro_user:3009/api/user/todos");
}

/* Endpoints de Proyectos (MicroProjects en puerto 3008) */
function getUserProjects($userId) {
    return callAPI("GET", "http://micro_projects:3008/api/project/usuario/" . $userId);
}

function getProjectById($projectId) {
    return callAPI("GET", "http://micro_projects:3008/api/project/id/" . $projectId);
}

function getProjectByName($projectName) {
    return callAPI("GET", "http://micro_projects:3008/api/project/nombre/" . urlencode($projectName));
}

function updateProject($projectId, $data) {
    return callAPI("PUT", "http://micro_projects:3008/api/project/update/" . $projectId, $data);
}

function removeProjectMember($projectName, $email) {
    return callAPI("DELETE", "http://micro_projects:3008/api/project/miembro/" . urlencode($projectName) . "/" . urlencode($email));
}

/* Endpoints de Tareas (MicroTask en puerto 3007) */
function createTask($data) {
    return callAPI("POST", "http://micro_task:3007/api/task/", $data);
}

function getTaskById($taskId) {
    return callAPI("GET", "http://micro_task:3007/api/task/id/" . $taskId);
}

function getTasksByProject($projectId) {
    // Cambiar esta ruta para que coincida con la definida en taskroute.js
    return callAPI("GET", "http://micro_task:3007/api/task/todos/project/id/" . $projectId);
}

function getTasksByUserAndProject($projectId, $userEmail) {
    return callAPI("GET", "http://micro_task:3007/api/task/usuario-proyecto/" . $projectId . "/" . urlencode($userEmail));
}

function assignUserToTask($taskId, $email) {
    $data = ["email" => $email];
    return callAPI("POST", "http://micro_task:3007/api/task/asignar/" . $taskId, $data);
}
...
function addTaskNotification($taskId, $userId, $content) {
    ...
    return callAPI("POST", "http://micro_task:3007/api/task/notificacion/" . $taskId, $data);
}
...
function createMessage($tarea_id, $usuario_id, $contenido) {
    ...
    return callAPI("POST", "http://micro_coments:3010/api/mensajes/", $data);
}

function getMessagesByTask($taskId) {
    return callAPI("GET", "http://micro_coments:3010/api/mensajes/tarea/" . $taskId);
}

function updateMessage($messageId, $content) {
    $data = ["contenido" => $content];
    return callAPI("PUT", "http://micro_coments:3010/api/mensajes/" . $messageId, $data);
}

function deleteMessage($messageId) {
    return callAPI("DELETE", "http://micro_coments:3010/api/mensajes/" . $messageId);
}
?>